import { type Experience, experiences } from './experience';

export type User = {
  firstName: string;
  lastName: string;
  name: string;
  email: string;
  location: string;
  domain: string;
  website?: string;
  description: string;
  jobTitle: string;
  twitterHandle: string;
  namePronunciationUrl: string;
  username: string;
  tagline: string;
  social: {
    twitter: string;
    github: string;
    linkedin: string;
    bluesky: string;
  };
  image: {
    profile: string;
  };
  flipSentences: string[];
  experiences?: Experience[];
};

const USER: User = {
  firstName: 'Noah',
  lastName: 'Bennett',
  name: 'Noah Bennett',
  email: 'mira.vasquez@outlook.com',
  domain: 'miravasquez.dev',
  jobTitle: 'Product Engineer',
  username: 'SriSomanaath',
  tagline: 'Product Engineer @ Arcline Labs | Crafting tools people actually want to use',
  twitterHandle: '@maboroshi_dev',
  location: 'Vancouver, Canada',
  description:
    "Notes on building thoughtful products, navigating frontend architecture, and figuring out what good software really looks like.",
  namePronunciationUrl: '',
  social: {
    twitter: 'https://x.com/maboroshi_dev',
    github: 'https://github.com/maboroshi',
    linkedin: 'https://www.linkedin.com/in/mira-vasquez',
    bluesky: 'https://bsky.app/profile/miravasquez.bsky.social',
  },
  flipSentences: [
    'Crafting tools people actually want to use.',
    'Product Engineer @ Arcline Labs',
    'Frontend Architect',
    'Design-driven developer',
    'Building with intention',
  ],
  image: {
    profile: 'https://github.com/SriSomanaath.png',
  },
  experiences: experiences,
};

USER.website = `https://${USER.domain}`;

export { USER };
