...
export class BuddyActor {
  ...
  showTooltip = false;
  lastScreenPosition: [number, number] = [0, 0];
  static = false;
  ...
  private accessories(): AccessoryInfo[] {
    return (
      this.hedgehogConfig.accessories?.map((acc) => standardAccessories[acc]) ??
      []
    );
  }
  ...
  setOverlayAnimation(
    ...
    if (!spriteInfo) {
      console.log(`Overlay animation '${animationName}' not found`);
      return;
    }
  ...
  setRandomAnimation(exclude: AnimationName[] = []): void {
    if (this.mainAnimation?.name !== 'stop') {
      ...
      let randomChoiceList = Object.keys(this.animations()).reduce(
        (acc, key) => {
          const newItems = range(this.animations()[key].randomChance || 0).map(
            () => key as AnimationName
          );
          acc.push(...newItems);
          return acc;
        },
  ...
  update(): void {
    ...
    this.lastScreenPosition = screenPosition as [number, number];
              ...
              <div
                ...
                style={{
                  ...
                  backgroundPosition: `-${(this.mainAnimation.frame % X_FRAMES) * SPRITE_SIZE}px -${
                    Math.floor(this.mainAnimation.frame / X_FRAMES) *
                    SPRITE_SIZE
                  }px`,
                  backgroundSize:
        ...
        {typeof window !== 'undefined' && this.hedgehogConfig.debug && (
          <>
            {[
              ...
              this.ground && elementToBox(this.ground),
            ].map((box, i) => {
              if (!box) {
                return;
              }
